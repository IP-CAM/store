...
<div id="content">
  <div class="page-header">
    <div class="container-fluid">
      ...
      <h1><?php echo $heading_title_update; ?></h1>
      <ul class="breadcrumb">
  ...
  <div class="container-fluid">
    ...
    <div class="panel panel-default">
      <div class="panel-heading">
        <h3 class="panel-title"><i class="fa fa-pencil"></i> <?php echo $text_edit_scheme; ?></h3>
      </div>
      <div class="panel-body">      
	  <form action="<?php echo $edit; ?>" method="post" enctype="multipart/form-data" id="form-mta-update" class="form-horizontal" onsubmit="javascript:return false;">
      <input type="hidden" name="id" value="<?php echo $s_id;?>" />	  
		<div class="form-group required">
			<label class="col-sm-2 control-label" for="input-name"><?php echo $entry_name; ?></label>
            <div class="col-sm-10">
				<input type="text" name="name" value="<?php echo $s_name; ?>" id="input-name" class="form-control" />
            </div>
        </div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="input-description"><?php echo $entry_description; ?></label>
            <div class="col-sm-10">
				<textarea name="description" id="input-description" class="form-control"><?php echo $s_description;?></textarea>
            </div>
        </div>
		<div class="form-group">
            ...
            <div class="col-sm-10">
			  <label class="radio-inline">
                <input type="radio" name="is_default" value="1" <?php if ($s_is_default) { ?>checked="checked" <?php } ?>/>
                <?php echo $text_yes; ?>
			  </label>
			  <label class="radio-inline">
                <input type="radio" name="is_default" value="0" <?php if (!$s_is_default) { ?>checked="checked" <?php } ?>/>
                <?php echo $text_no; ?>
			  </label>
            </div>
        </div>	  
		...
		<div class="form-group">
            ...
            <div class="col-sm-10">
			  <label class="radio-inline">
                <input type="radio" name="before_shipping" value="1"<?php if ($s_before_shipping) { ?> checked="checked"<?php } ?> />
                <?php echo $text_yes; ?>
			  </label>				
			  <label class="radio-inline">
                <input type="radio" name="before_shipping" value="0"<?php if (!$s_before_shipping) { ?> checked="checked"<?php } ?> />
                <?php echo $text_no; ?>
			  </label>
            </div>
        </div>	
...
$(document).ready(function () { 	
	...
	var change_tl = function() {
      ...
      $.get(document.location.href.replace('edit', 'check_scheme_name')+'&name='+encodeURIComponent($('input[name="name"]').val())+'&id='+encodeURIComponent($('input[name="id"]').val()), function(r) {
      	if(!(parseInt(r))) return failform('<?php echo mta_jsstr($error_duplicate_name);?>');

      	jQuery.post(
      		document.location.href+'&',      		
      		$('#form-mta-update').serialize(),
      		function(d) {
      			if(d == '1') {
      				document.location.href = document.location.href.replace('/edit', '').replace(/\&id\=\d+/, '');
      				return;
      			}
      			return failform(d);
      		},
		...
		var populate_commission = function(c, ad, ap) {
			for(var i = 0; i < c.length; i++) {
				...
				for(var i2 = 0; i2 < c[i].length; i2++) {				
					...
					if(ad[i][i2]) {
						$('input[name="tiers'+ip+'_level'+i2p+'_autoadd"][value="0"]').prop('checked', false);
						$('input[name="tiers'+ip+'_level'+i2p+'_autoadd"][value="1"]').prop('checked', true);
					} else {
						$('input[name="tiers'+ip+'_level'+i2p+'_autoadd"][value="1"]').prop('checked', false);
						$('input[name="tiers'+ip+'_level'+i2p+'_autoadd"][value="0"]').prop('checked', true);
					}
				}		
				if(ap[i]) {
					$('input[name="level'+ip+'_autoapprove"][value="0"]').prop('checked', false);
					$('input[name="level'+ip+'_autoapprove"][value="1"]').prop('checked', true);
				} else {
					$('input[name="level'+ip+'_autoapprove"][value="1"]').prop('checked', false);
					$('input[name="level'+ip+'_autoapprove"][value="0"]').prop('checked', true);
				}
			}	
		}
