		...
		//+mod by yp end
			...
			if ($this->config->get('config_account_id')) {
				...
				if ($information_info && !isset($this->request->post['agree'])) {
					$json['error']['warning'] = (isset($json['error']['warning']) && utf8_strlen($json['error']['warning']) > 0 ? $json['error']['warning'] . '<br />' : '') . sprintf($this->language->get('error_agree'), $information_info['title']);
				}
			}
